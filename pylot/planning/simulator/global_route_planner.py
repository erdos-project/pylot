...
The module is based on the implementation from the CARLA codebase.
"""
...
from pylot.utils import LaneChange, LaneType, RoadOption


def vector(location_1, location_2):
    """Computes the unit vector from location_1 to location_2."""
...
class GlobalRoutePlanner(object):
    ...
    def _build_graph(self):
        ...
        road_id_to_edge = dict()  # Map {road_id: {lane_id: edge, ... }, ... }
        ...
        for segment in self._topology:
            ...
            road_id, section_id, lane_id = \
                entry_wp.road_id, entry_wp.section_id, entry_wp.lane_id
    ...
    def _localize(self, location):
        ...
        Args:
            location: Location to be localized in the graph.
        ...
        try:
            ...
            print("Failed to localize! : ", "Road id : ", waypoint.road_id,
                  "Section id : ", waypoint.section_id, "Lane id : ",
                  waypoint.lane_id, "Location : ",
        ...
        for segment in self._topology:
            ...
            for waypoint in segment['path']:
                if not segment['entry'].is_junction:
                    ...
                    if waypoint.right_lane_marking.lane_change.real in \
                       [LaneChange.RIGHT.value, LaneChange.BOTH.value] and \
                       not right_found:
                        next_waypoint = waypoint.get_right_lane()
                        if next_waypoint is not None and \
                           next_waypoint.lane_type.real == \
                           LaneType.DRIVING.value and \
                           waypoint.road_id == next_waypoint.road_id:
                            ...
                            if next_segment is not None:
                                ...
                                right_found = True
                    if waypoint.left_lane_marking.lane_change.real in \
                       [LaneChange.LEFT.value, LaneChange.BOTH.value] and \
                       not left_found:
    ...
    def _path_search(self, origin, destination):
        """This function finds the shortest path connecting origin and
        destination using A* search with distance heuristic.

        Args:
            origin: Location object of start position.
            destination: Location object of of end position.

        Returns:
            Path as list of node ids (as int) of the graph connecting origin
            and destination
        """
    ...
    def _successive_last_intersection_edge(self, index, route):
        """Returns the last successive intersection edge from a starting
        index on the route.
        ...
        for node1, node2 in [(route[i], route[i + 1])
            ...
            if candidate_edge['type'] == RoadOption.LANE_FOLLOW and \
    ...
    def _turn_decision(self, index, route, threshold=math.radians(35)):
        """ Returns the turn decision (RoadOption) for pair of edges
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID and \
               ...
               next_edge['intersection']:
                ...
                calculate_turn = \
                    current_edge['type'] == RoadOption.LANE_FOLLOW and \
    ...
    def abstract_route_plan(self, origin, destination):
        """ Generates the route plan.

        Args:
            origin: Location object of the route's start position.
            destination: Location object of the route's end position.

        Returns:
            List of turn by turn navigation decisions as RoadOption elements.
        """
    ...
    def trace_route(self, origin, destination):
        ...
        route_trace = []
        route = self._path_search(origin, destination)
        cur_waypoint = self._dao.get_waypoint(origin)
        dst_wp = self._dao.get_waypoint(destination)
        ...
        for i in range(len(route) - 1):
            ...
            if edge['type'] != RoadOption.LANE_FOLLOW and \
               edge['type'] != RoadOption.VOID:
                route_trace.append((cur_waypoint, road_option))
                ...
                if next_edge['path']:
                    closest_index = self._find_closest_in_list(
                        cur_waypoint, next_edge['path'])
                    closest_index = min(
                        len(next_edge['path']) - 1, closest_index + 5)
                    cur_waypoint = next_edge['path'][closest_index]
                else:
                    cur_waypoint = next_edge['exit_waypoint']
                route_trace.append((cur_waypoint, road_option))

            else:
                path = path + [edge['entry_waypoint']
                               ] + edge['path'] + [edge['exit_waypoint']]
                closest_index = self._find_closest_in_list(cur_waypoint, path)
                for waypoint in path[closest_index:]:
                    cur_waypoint = waypoint
                    route_trace.append((cur_waypoint, road_option))
                    if len(route) - i <= 2 and \
                       waypoint.transform.location.distance(
                           destination) < 2 * resolution:
                        break
                    elif (len(route) - i <= 2
                          and cur_waypoint.road_id == dst_wp.road_id
                          and cur_waypoint.section_id == dst_wp.section_id
                          and cur_waypoint.lane_id == dst_wp.lane_id):
