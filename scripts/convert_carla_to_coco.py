"""
Takes in a folder of Carla observations (images, bounding box annotations) and converts them
into COCO format (which can be used by e.g. CenterNet).
...
def main(_):
    categories=['pedestrian', 'vehicle', 'traffic sign']
    ...
    idxs = [int(s[7:-5]) for s in os.listdir(FLAGS.data) if s.startswith("bboxes")]
    print("COUNT:", len(idxs))
    for idx in idxs:
        image_info = {'file_name': 'carla-{}.png'.format(idx),
                      "id": int(idx)}
